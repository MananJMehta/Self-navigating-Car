...
//#include "../L5_Application/periodic_scheduler/periodic_callback.cpp"
//#include "periodic_callback.cpp"

// Include for C++
using namespace cgreen;
#include "../L5_Application/periodic_scheduler/period_callbacks.cpp"
...
Ensure(first_map_get_values)
{
    SENSOR_DATA_t x;
    x={0,1,1,1,0,1,1,1,1};
    assert(map_get_value('F', x), is_equal(0));
    //assert(map_get_value('c', x), is_equal(0));
    /*std::pair<uint8_t , uint8_t> s;
    s=update_lanes(x);*/
}
...
int main(int argc, char **argv) 
{
    ...
    add_suite(suite, first_map_get_values);
